module.exports = (app) => {
  ...
  router.get("/", async (req, res) => {
    ...
    console.log("module", req.params, model);
  ...
  router.put("/:id", async (req, res) => {
    let info = await req.module.findById(req.params.id);
    let model = await req.module.findByIdAndUpdate(req.params.id, {
      ...info,
      ...req.body,
    });
  ...
  app.post("/admin/login", async (req, res) => {
    ...
    // 查找用户是否存在
    const user = await Admins.findOne({ username: username }).select(
      "+password"
    );
