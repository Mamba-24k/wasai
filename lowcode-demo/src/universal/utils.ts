...
export const saveSchema = async (scenarioName: string = 'index') => {
  ...
  const searchParse = qs.parse((window.location.search || '?').split('?')[1]);
  await fetch(`http://localhost:3000/admin/rest/pages/${searchParse.pageCode}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ pageSchema: JSON.stringify(project.exportSchema(TransformStage.Save)) }),
  });
...
export const getProjectSchemaFromLocalStorage = async (scenarioName: string) => {
  ...
  const searchParse = qs.parse((window.location.search || '?').split('?')[1]);
  const pageParse: any = await fetch(
    `http://localhost:3000/admin/rest/pages/${searchParse.pageCode}`,
    {
      method: 'GET',
    },
  ).then((res) => res.json());
  console.log(pageParse, 'pageParse');
  return JSON.parse(pageParse?.pageSchema || '{}');
};
